...
echo "=== Building Lean Whisper Transcriber AppImage v$VERSION (with CUDA support) ==="
...
echo "Creating AppRun entry point with CUDA detection..."
...
echo "Creating CUDA helper module..."
...
def configure_torch_for_cuda():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            logging.info(f"CUDA is available with {device_count} device(s): {device_name}")
            return True
        else:
            logging.info("PyTorch reports CUDA is not available despite NVIDIA drivers")
            return False
...
# Install packages with minimal PyTorch that can use system CUDA if available
pip install --no-cache-dir --upgrade pip wheel setuptools
# Install minimal PyTorch with CPU-only support, system will use its own CUDA
...
# 10. Bundle only essential system libraries
...
echo "It will use the system's CUDA if available, otherwise fall back to CPU."
echo "New in v0.8: Audio level visualization and improved transcription quality"
echo "New in v0.9: Trying to ammend CUDA initialization issues"
